    ...
    def run(self):
        ...
        while(proc_dsc<total_dsc):                                                  
            ...
            if(next_step['machine']==self.ID):   #被叫到
                ...
                wait_time[self.ID] = processing_time + int(time.time())                               #wait time edited
                ...
                wait_time[self.ID] = int(time.time())                                               ##wait time edited
    ...
    def run(self):
        ...
        while(proc_dsc<total_dsc): 
            ...
            for i in range(len(abs_wait_times)):
                t = abs_wait_times[i] - int(time.time())
                if(t > 0):
                    self.wait_time[i] = t/time_unit
                else:
                    self.wait_time[i] = 0
...
# print(oga_fjs("bran/Mk01.fjs",time_unit_param = 1))
