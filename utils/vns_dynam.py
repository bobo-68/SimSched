...
def total_flow_time(ind, tasks, w_t = [0 for i in range(100)], w_t_j = [0]*100):
    ...
    # print(sum(job_ok_time))
    return sum(job_ok_time)

def search(ind, tasks, iter_max, nmax, m_num, w_t = [0 for i in range(100)]):
		...
		while(k<3):
			...
			while(n<nmax):
				...
				if(l==0):
					...
					# print("local se2")
					xp = se2(xd, len(tasks))
				if(total_flow_time(xp, tasks, w_t) <= total_flow_time(xd, tasks, w_t)):
					xd = xp
				else:
					l = (l+1)%9
				n += 1
			if(total_flow_time(xd, tasks, w_t) < total_flow_time(x, tasks, w_t)):
				x = xd
				k = 0
				converg = 0
			else:
