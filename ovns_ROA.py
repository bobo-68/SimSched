...
from utils.dispatching_rules import SPT_sol, FIFO_sol, AT_sol
...
class Machine(threading.Thread):
    ...
    def run(self):
        global task_list, tasks_just_done, real_records, theo_records, wait_time, wait_time_job, next_step, proc_dsc
        while(proc_dsc<total_dsc):                                                  
            ...
            if(next_step['machine']==self.ID):   #被叫到
                ...
                wait_time[self.ID] = processing_time + time.time()
                wait_time_job[job_index] = processing_time + time.time()                               #wait time edited
                ...
                wait_time[self.ID] = time.time()
                wait_time_job[job_index] = time.time()                                               ##wait time edited
...
class Scheduler(threading.Thread):
    ...
    def evolve(self, mut_rate = 0.1, cross_rate = 0.7, elite = 2, eliminate = 0):
        ...
        while(n<100):
            ...
            if(genetic.total_time(xp, self.tasks, self.wait_time, self.wait_time_job) <= genetic.total_time(xd, self.tasks, self.wait_time, self.wait_time_job)):
                xd = xp 
            else:
                l = (l+1)%9
            n += 1
        if(genetic.total_time(xd, self.tasks, self.wait_time, self.wait_time_job) < genetic.total_time(x, self.tasks, self.wait_time, self.wait_time_job)):
            self.best = xd
            self.k = 0
        else:
            self.k = (self.k+1)%3
...
print(oga_fjs("bran/Mk08.fjs",time_unit_param = 1))
